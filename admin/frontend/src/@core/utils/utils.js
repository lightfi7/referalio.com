...
export const setSession = (accessAdminToken, name) => {
  if (accessAdminToken && name) {
    localStorage.setItem('accessAdminToken', accessAdminToken)
    localStorage.setItem('currentUserName', name)
    axios.defaults.headers.common.Authorization = `Bearer ${accessAdminToken}`
    const { exp, iat } = jwtDecode(accessAdminToken)
